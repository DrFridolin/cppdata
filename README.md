...
- [stack&lt;T&gt;](#stackt) : General-purpose stack implemented with a singly-linked list ([node&lt;T, false&gt;](#nodet-d)).
- [queue&lt;T&gt;](#queuet) : General-purpose queue implemented with a doubly-linked list ([linked_list&lt;T, true&gt;](#linked_listt-d)).
- [graph_node&lt;T&gt;](#graph_nodet) : Self-referential graph nodes for acyclic and unweighted connected graphs and trees.
- [graph&lt;T&gt;](#grapht) : Weighted and unweighted graphs that track [graph_node&lt;T&gt;](#graph_nodet) pointers in a [dyn_array&lt;graph_node&lt;T&gt;*&gt;](#dyn_arrayt) member.
...
- [hash_table&lt;K, V&gt;](#hash_tablek-v) : Type-aware hash table implemented with a fixed-size raw array of singly-linked [node&lt;T, false&gt;](#nodet-d) chains.
...
For detailed usage patterns, method signatures, and exhaustive code snippets covering all available features, compile and inspect the executables in tests/ directory. These serve both as unit tests and comprehensive usage examples. Build with CMake:
```
cmake -B build -S . && cmake --build build --target test_dyn_array
```
...
|`resize(size_t n) : dyn_array<T>&` | For n>size, pads the array to size n with default T() values. For n<size, shrinks the array and copies old data without its tail portion.  | O(N) | O(N) | O(N) |
...
|`operator = (const dyn_array<T>&) : dyn_array<T>&` | Assignment operator performs deep copy from source. | O(N) | O(N) | O(N) |
...
Node-only class for singly (D=false) and doubly (D=true) linked lists. Base class __node_base&lt;T, D&gt;__ only holds node value and next_ pointer for singly linked lists, and introduces a prev_ pointer via template specialization for doubly-linked lists. It also establishes both read/write and const access to its member pointers. Trying to call prev() on a singly linked node chain will result in undefined behavior.

All other operations are implemented in its child class __node&lt;T, D&gt;__, which is a self-referential linked list representation where chains are handled from their head node pointers. Nodes are restricted to heap memory instantiation and are created through the static create method with an initial value. A node instance owns the memory for the entire chain in front of it. Methods that can change the head node return a pointer to the node occupying the head position at the time of return, which will be lost unless captured at call site. Tail-editing methods return a pointer to tail position in the same way if their head nodes are safe, which eliminates the need for an extra traversal for post processing the new tail in some cases. It also allows [linked_list&lt;T, D&gt;](#linked_listt-d) class to easily update its tail pointer after running tail-editing node method.

Implements four sorting algorithms that are native to linked lists; algorithms utilizing frequent random access are deemed incompatible and/or suboptimal. The variant of merge_sort in __node&lt;T, D&gt;__ reaches better asymptotic worst-case time complexity than its [dyn_array&lt;T&gt;](#dyn_arrayt) variant, and uses less space by foregoing an auxiliary structure. The use of a fully iterative variant brings down space complexity from O(logN) down to O(1). Therefore, merge_sort is the optimal way to sort a __node&lt;T, D&gt;__ chain or a [linked_list&lt;T, D&gt;](#linked_listt-d) object.
...
|`operator = (const node<T, D>& source) : node<T, D>&` | Performs deep copy of the source node chain. | O(N+M) | O(N+M) | O(N) |
|`operator = (std::initializer_list<T> list) : node<T, D>&` | Creates a node chain from values in the initializer list. | O(N+M) | O(N+M) | O(N) |
|`operator = (const dyn_array<T>& source) : node<T, D>&` | Creates a node chain from values in the dyn_array. | O(N+M) | O(N+M) | O(N) |
...
|`operator <= (const linked_list<T, D>& other) const : bool` | Uses node<T, D> criteria and operator to run an equivalent comparison from head nodes. | O(1) | O(N) | O(1) |
|`operator >= (const linked_list<T, D>& other) const : bool` | Uses node<T, D> criteria and operator to run an equivalent comparison from head nodes. | O(1) | O(N) | O(1) |
|`operator = (const linked_list<T, D>& source) : linked_list<T, D>&` | Performs deep copy from another linked_list. | O(N+M) | O(N+M) | O(N) |
|`operator = (std::initializer_list<T> list) : linked_list<T, D>&` | Creates a linked list from values in the initializer list. | O(N+M) | O(N+M) | O(N) |
|`operator = (const node<T, D>& source) : linked_list<T, D>&` | Creates a linked from a node chain. | O(N+M) | O(N+M) | O(N) |
|`operator = (const dyn_array<T>& source) : linked_list<T, D>&` | Creates a node chain from values in the dyn_array. | O(N+M) | O(N+M) | O(N) |
...
General-purpose stack that runs a [node&lt;T, false&gt;](#nodet-d) chain under the hood and is mostly a thin wrapper around it that exposes a clean stack interface. Provides O(1) access to last inserted value. Keeps track of its own size and represents its empty state by nullifying its node pointer member. Owns the memory for its linked list member.
...
|`operator <= (const stack<T>& other) const : bool` | Compares node chains directly using node<T, false> operator. | O(1) | O(N) | O(1) |
|`operator >= (const stack<T>& other) const : bool` | Compares node chains directly using node<T, false> operator. | O(1) | O(N) | O(1) |
|`operator = (const stack<T>& source) : stack<T>&` | Performs a deep copy of source with its node chain. | O(N+M) | O(N+M) | O(N) |
|`operator = (std::initializer_list<T> list) : stack<T>&` | Clears stack and directly assigns the initializer list to its node member. | O(N+M) | O(N+M) | O(N) |
|`operator = (const node<T, false>* source) : stack<T>&` | Clones the source node chain into its member. | O(N+M) | O(N+M) | O(N) |
|`operator = (const dyn_array<T>& source) : stack<T>&` | Clears stack and directly assigns dyn_array to its node member. | O(N+M) | O(N+M) | O(N) |
...
General-purpose queue that runs a [linked_list&lt;T, true&gt;](#linked_listt-d) under the hood for O(1) tail access, and is mostly a thin wrapper around it that exposes a clean queue interface. Provides O(1) access to earliest inserted value, as well as the last inserted value since it comes for free with [linked_list&lt;T, true&gt;](#linked_listt-d) and is often useful. Keeps track of its own size and represents its empty state with an empty linked list member. Owns its memory but manages no dynamically allocated memory on its own, deferring instead to compiler-built defaults for intra-class assignments and copy construction.
...
|`operator <= (const queue<T>& other) const : bool` | Compares linked lists directly using linked_list<T, true> operator. | O(1) | O(N) | O(1) |
|`operator >= (const queue<T>& other) const : bool` | Compares linked lists directly using linked_list<T, true> operator. | O(1) | O(N) | O(1) |
|`operator = (std::initializer_list<T> list) : queue<T>&` | Clears queue and directly assigns the initializer list to its linked list member. | O(N+M) | O(N+M) | O(N) |
|`operator = (const linked_list<T, true>& source) : queue<T>&` | Assigns the source linked list directly to its member. | O(N+M) | O(N+M) | O(N) |
|`operator = (const dyn_array<T>& source) : queue<T>&` | Clears queue and directly assigns dyn_array to its linked list member. | O(N+M) | O(N+M) | O(N) |
...
Due to limitations both inherent and deliberate in its design, __graph_node&lt;T&gt;__ does not support disconnected subgraphs, cycles, loops or bidirectional searches on directed graphs. Graphs with disconnected components can only be represented as a set of separate __graph_node&lt;T&gt;__ chains. Chains with directed edges can end up with nodes that are unreachable from any other node. Cyclic graphs and trees can be created, but traversal attempts will result in infinite loops, as support for loops and cycles are delegated to compositional classes like [graph&lt;T&gt;](#grapht) that globally manage their connectivity structure.
...
|`graph_node(size_t n)` | Instantiates graph_node with adjacency list of size n without value initialization. Sets node value to default T. | O(A) | O(A) | O(A) |
|`graph_node(const T& v, size_t n)` | Initialize graph_node with its value and adjacency list size. | O(A) | O(A) | O(A) |
...
|`operator = (std::initializer_list<graph_node<T>*> list) : graph_node<T>&` | Sets initializer list elements as node's neighbors. Caller is responsible for managing lifetimes and avoiding cycles/loops. | O(A) | O(A) | O(A) |
|`resize_adj(size_t n) : graph_node<T>&` | Resizes the dyn_array holding node's neighbor pointers. | O(A) | O(A) | O(A) |
...
- 2D list/array to rebuild the edge structure from scratch without changing any node values, but adding and removing them as needed to comply with the size dictated by the edge matrix.
...
|`graph(std::initializer_list<T> node_vals)` | Constructs graph from a list of node values. | O(N) | O(N) | O(N) |
|`graph(const dyn_array<T>& node_vals)` | Constructs graph from a dyn_array of node values. | O(N) | O(N) | O(N) |
...
|`graph(const graph<T>& source)` | Copy constructor performs deep copy of nodes and edges. | O(N<sup>2</sup>) | O(N<sup>2</sup>) | O(N<sup>2</sup>) |
...
|`operator = (const graph<T>& source) : graph<T>&` | Performs deep copy of source nodes and edge structure. | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>) |
|`operator = (sq_init_list adj_mat) : graph<T>&` | Rebuilds graph edges from 2D initializer list. Preserves existing node values and indices while removing excess nodes. If matrix is larger, creates new default-valued nodes with no neighbors. | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>) |
|`operator = (const dyn_array<dyn_array<int> >& adj_mat) : graph<T>&` | Rebuilds graph edges from 2D dyn_array. Preserves existing node values and indices while removing excess nodes. If matrix is larger, creates new default-valued nodes without any neighbors. | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>) |
|`operator = (std::initializer_list<T> node_vals) : graph<T>&` | Reassigns node values from 1D initializer list in node index order. If list is larger than the node array, creates new default-valued nodes with no neighbors. If list is smaller, only the indices within list bounds will have their values reassigned. | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>) |
|`operator = (const dyn_array<T>& node_vals) : graph<T>&` | Reassigns node values from 1D dyn_array in node index order. If dyn_array is larger than the node array, creates new default-valued nodes with no neighbors. If dyn_array is smaller, only the indices within dyn_array bounds will have their values reassigned. | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>) |
|`operator = (graph_node<T>* subgraphs) : graph<T>&` | Clears and rebuilds graph as a copy of the subgraph reachable from the assigned graph_node. | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>+M<sup>2</sup>) | O(N<sup>2</sup>) |
...
|`add_node(const T& val) : size_t` | Creates a graph_node on heap memory holding the given value and no neighbors, then adds it to the graph and returns its index. | O(N) | O(N<sup>2</sup>) | O(N) |
|`add_node(graph_node<T>* subgraph) : size_t` | Copies the given graph_node and all nodes connected to and reachable from it into the graph. Changes the indices of the subgraph nodes to match them to the indices of their copies in the graph. Returns the index assigned to the input node. | O(NS+ S<sup>2</sup>+E<sub>S</sub>) | O(N<sup>2</sup>S+ S<sup>2</sup>+E<sub>S</sub>) | O(NS+ S<sup>2</sup>) |
...
Level-order traversals, on the other hand, are iterative in nature and require a [queue&lt;T&gt;](#queue-t) to track nodes; therefore they are powered by a separate engine called stratify. Following a similar idea and using the exact same lambda signature for customization, stratify deploys a [queue&lt;T&gt;](#queue-t) to traverse and separate the tree levels, running the lambda on each dequeued node with current level and parent queue pointer passed as the two intptr_t arguments. Lambda returns of -1 will trigger an early termination check with nullptr as before.
...
For its core virtual methods like search, insert and remove, the goal of __binary_tree&lt;T, P&gt;__ is to provide maximally-useful default behaviors that are well-defined on all binary trees. For example, even though there is no inherent insertion order defined for a general __binary_tree&lt;T, P&gt;__, `binary_tree<T, P>::insert`performs level-order left-to-right insertion and guarantees to preserve or approach both balance and completeness with each insertion. This method ends up forming the backbone of [heap_tree&lt;T, P, M&gt;](#heap_treet-p-m) insertions.

Cyclic binary trees are currently not supported.
...
|`operator = (const binary_tree<T, P>& src) : binary_tree<T, P>&` | Clones source tree starting from the given node. | O(N+M) | O(N+M) | O(N) |
|`operator = (std::initializer_list<T> list) : binary_tree<T, P>&` | Turns init list to dyn_array, then rebuilds tree from it. | O(N+M) | O(N+M) | O(N) |
|`operator = (const dyn_array<T>& src) : binary_tree<T, P>&` | Level-order rebuilds complete binary tree from dyn_array. | O(N+M) | O(N+M) | O(N) |
...
|`find(const T& val) const : dyn_array<const binary_tree<T, P>*>` | Recursively finds all nodes in the tree with values matching query. | O(N) | O(N) | O(H+M) |
...
|`is_subtree(const binary_tree<T, P>* bt_ptr) const : bool` | Returns true if calling node's tree is a subtree of bt_ptr. | O(N+MS) | O(NS) | O(H+M) |
...
A type of [binary_tree&lt;T, P&gt;](#binary_treet-p) specialized for efficient search. It plays the same role for all search trees that [binary_tree&lt;T, P&gt;](#binary_treet-p) does for all binary trees by providing the following set of foundational capabilities to its descendants:
...
To make the newly-introduced count_ member fully customizable via polymorphic interjections and overrides by descendants, __binary_search_tree&lt;T, P&gt;__ encapsulates all const and non-const access to this member in simple virtual methods.

The balance method traverses the tree in-order to get sorted values, then recursively rebuilds a balanced and complete __binary_search_tree&lt;T, P&gt;__ from the sorted array of values. It provides completeness guarantee by adjusting the mid-value selection heuristic and skew the recursion in favor of the left branch just enough to ensure nodes at the deepest level are accumulated contiguously on the left side. Same recursion is deployed to build search trees from any [dyn_array&lt;T&gt;](#dyn_arrayt) simply by sorting it prior to recursive buildup; or indeed from any [binary_tree&lt;T, P&gt;](#binary_treet-p) since they can be a serialized into node value arrays with a full traversal.
...
Insertions and removals swap node values rather than node objects, so node addresses are not guaranteed to preserve their values during state-changing operations. To protect the integrity of its internal state, __binary_search_tree&lt;T, P&gt;__ exposes the alternative, const-only accessor set publicly (left_ch(), right_ch() and parent()). Manual modifications to its structure are highly discouraged but possible with dynamic casts to [binary_tree&lt;T, P&gt;](#binary_treet-p) pointers to expose the mutators. Use to avoid self-balancing overhead when balance scheduling is handled externally.
...
|`operator = (const binary_search_tree<T, P>& src) : binary_search_tree<T, P>&` | Runs the binary_tree operator, which clones tree structure and copies all members with RTTI. | O(N+M) | O(N+M) | O(N) |
...
Inherited methods do a lot of the heavy lifting in __avl_tree&lt;T, P&gt;__'s handling of its core search functions, local node removal logic, assignments from other types of binary trees, recursive rebuilding from value arrays, duplicate value handling and on-demand balancing for its completeness guarantee.
...
|`operator = (const binary_search_tree<T, P>& src) : avl_tree<T, P>&` | Runs binary_tree operator, then balances with binary_search_tree method. | O(N+M) | O(N+M) | O(N) |
...
|`operator = (const binary_search_tree<T, true>& src) : red_black_tree<T>&` | Runs avl_tree operator, then colorizes the entire tree. | O(N+M) | O(N+M) | O(N) |
|`operator = (const avl_tree<T, true>& src) : red_black_tree<T>&` | Runs binary_tree operator, then colorizes the entire tree. | O(N+M) | O(N+M) | O(N) |
...
|`operator = (std::initializer_list<T> list) : heap_tree<T, P, M>&` | Builds a dyn_array from initializer list, then assigns it to itself with dyn_array assignment operator. | O(N+M) | O(N+M) | O(N) |
|`operator = (const dyn_array<T>& src) : heap_tree<T, P, M>&` | Assigns to self with binary_tree operator to rebuild as a complete tree, then heapifies itself. | O(N+M) | O(N+M) | O(N) |
|`operator = (const binary_tree<T, P>& src) : heap_tree<T, P, M>&` | Assigns complete trees with binary_tree operator, then heapifies itself. All others are traversed and rebuilt from dyn_array. | O(N+M) | O(N+M) | O(N) |
|`operator = (const binary_search_tree<T, P>& src) : heap_tree<T, P, M>&` | Builds a dyn_array from binary_search_tree's node values including duplicates, then assigns it to itself with dyn_array assignment operator. | O(N+M) | O(N+M) | O(N) |
|`operator = (const heap<T>& hp) : heap_tree<T, P, M>&` | Rebuilds heap_tree by duplicating the structure of an array-based heap. | O(N+M) | O(N+M) | O(N) |
...
Implements array counterparts of all standard heap operations in a manner that is consistent with [heap_tree&lt;T,P,M&gt;](#heap_treet-p-m).

When constructed from or assigned a [dyn_array&lt;T&gt;](#dyn_arrayt) or initializer list, __heap&lt;T&gt;__ copies values to its internal array, then heapifies it by sifting down each array index corresponding to a non-leaf node. Efficiently handles assignments to and from [heap_tree&lt;T,P,M&gt;](#heap_treet-p-m) by leveraging their matching structures.
...
|`heap(std::initializer_list<T> list, bool is_max)` | Assigns the initializer list of values to its internal dyn_array directly, then sifts down each element starting from last non-leaf node going backwards. | O(N) | O(N) | O(N) |
|`heap(const dyn_array<T>& source, bool is_max)` | Assigns the dyn_array to its heap's dyn_array member directly, then sifts down each element starting from last non-leaf node going backwards. | O(N) | O(N) | O(N) |
...
|`operator = (std::initializer_list<T> list) : heap<T>&` | Builds a dyn_array from initializer list, then assigns it to itself using dyn_array assignment operator. | O(N+M) | O(N+M) | O(N) |
|`operator = (const dyn_array<T>& arr) : heap<T>&` | Assigns dyn_array directly to its internal member, then heapifies it. | O(N+M) | O(N+M) | O(N) |
|`operator = (heap_tree<T, P, M>* hpt) : heap<T>&` | Clears and rebuilds heap from a heap_tree of any type. | O(N+M) | O(N+M) | O(N) |
...
A specialized, tree-structured [graph_node&lt;char&gt;](#graph_nodet) that implements a general-purpose prefix tree. Always maintains a root node with '~' character as the "start node", even when empty. Marks the ends of valid phrases with `\0` null termination character, therefore cannot handle string inputs containing this character. All other characters are valid __trie__ node values.

Like its parent class, __trie__ is node-only and self-referential. Following in the footsteps of [node&lt;T, D&gt;](#nodet-d) and [binary_tree&lt;T, P&gt;](#binary_treet-p), __trie__ restricts itself to heap memory with static create methods and protected constructors. Provides extensive support for std::string in its static create methods and assignment operators. Owns and manages all memory for its downstream chain.

Use to efficiently store, access and manipulate vocabularies and lexicons, searh for string prefixes and get string autocomplete suggestions.

Notation for trie big-O:
...
- D: Maximum degree (number of children) of any node (D ≤ A).
...
|`static create(const std::string& str) : trie*` | Heap-instantiate trie with a single string by breaking it up at spaces. | O(S) | O(S) | O(M) |
|`static create(const dyn_array<std::string>& dyn) : trie*` | Heap-instantiate trie with a dyn_array of strings with no breaking. | O(SL) | O(SL) | O(M) |
|`~trie()` | Destructor recursively deletes all descendant nodes. | O(N) | O(N) | O(logN) |
|`clone() const : trie*` | Heap-instantiates an identical trie, returns pointer to its root node. | O(N) | O(N) | O(N) |
...
|`operator = (const trie& source) : trie&` | Clears trie, then clones the source trie's structure. | O(N) | O(N) | O(N) |
|`operator = (const std::string& str) : trie&` | Clears trie, breaks up the string into space-separated words, then inserts each word one by one. | O(N+S) | O(N+S) | O(M) |
|`operator = (std::initializer_list<std::string> list) : trie&` | Clears trie, builds a dyn_array from initializer list, then assigns it to itself using dyn_array assignment operator. | O(N+SL) | O(N+SL) | O(M) |
|`operator = (const dyn_array<std::string>& source) : trie&` | Clears trie, then inserts dyn_array strings one by one. | O(N+SL) | O(N+SL) | O(M) |
...
|`insert(const std::string& str) : trie&` | Inserts a string into trie, creating new nodes as needed. | O(L) | O(L) | O(L) |
...
| pointer | uintptr_t | fowler-noll-vo, jenkins or murmur depending on PtrHashType |
...
| Method Signature | Description | Average | Worst | Space |
|------------------|-------------|:-------:|:-------:|:-----:|
|`hash_table()` | Default constructor uses prime number 1009 for raw array size and FNV for pointer hash function. | O(1) | O(1) | O(T) |
|`hash_table(size_t table_size)` | Constructs hash_table with given array size. Uses FNV for pointer hash function. | O(T) | O(T) | O(T) |
|`hash_table(size_t table_size, PtrHashType ptr_h)` | Constructs hash_table given array size and pointer hash function. | O(T) | O(T) | O(T) |
|`~hash_table()` | Destructors calls delete on each linked list head, then deallocates the raw array member. | O(N+T) | O(N+T) | O(N/T) |
...
|`clear() : hash_table<K, V>&` | Deletes all linked lists and nullifies raw array elements. | O(N+T) | O(N+T) | O(N/T) |
