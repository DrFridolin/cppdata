...
Linked list wrapper class manages a [node&lt;T, D&gt;](#nodet-d) chain by continuously tracking its head and tail nodes, and represents its empty state by nullifying both. Provides a cleaner way to manage and interact with linked list chains. Owns the memory for the full node chain and manages it via its head pointer, similar to how a [node&lt;T, D&gt;](#nodet-d) instance manages its downstream chain.
...
General-purpose queue that runs a doubly-linked [linked_list&lt;T, true&gt;](#linked_listt-d) under the hood for O(1) tail access, and is mostly a thin wrapper around it that exposes a clean queue interface. Provides O(1) access to earliest inserted value, as well as the last inserted value since it comes for free with linked lists. and is often useful. Keeps track of its own size and represents its empty state with an empty list_ member. Owns its memory but manages no dynamically allocated memory on its own, deferring instead to compiler-built defaults for intra-class assignments and copy construction, and to its [linked_list&lt;T, true&gt;](#linked_listt-d) member for recursive cleanups.
...
Like its parent class, __trie__ is node-only and self-referential. Following in the footsteps of [node&lt;T, D&gt;](#nodet-d) and [binary_tree&lt;T, P&gt;](#binary_treet-p), __trie__ restricts itself to heap memory with static create methods and protected constructors and recursively manages all its node memory. Provides extensive support for std::string in its static create methods and assignment operators.
